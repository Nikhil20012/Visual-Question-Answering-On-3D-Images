...
Adapted from https://github.com/facebookresearch/clevr-dataset-gen:
# --------------------- ---------------
...
import math, sys, random, argparse, json, os, tempfile, shutil, glob
import numpy as np
import get_b_box
...
# Input options
parser.add_argument('--base_scene_blendfile', default='data/base_scene_mv.blend',
...
parser.add_argument('--num_views', default=8, type=int,
    help="The number of views the camera traverses. higher -> better resolution ")
parser.add_argument('--min_objects', default=3, type=int,
    help="The minimum number of objects to place in each scene")
parser.add_argument('--max_objects', default=6, type=int,
    help="The maximum number of objects to place in each scene")
parser.add_argument('--min_dist', default=0.075, type=float,
...
parser.add_argument('--num_scenes', default=5, type=int,
    help="The number of scenes to create")
...
def main(args):
  ...
  mask_template = '%s%%0%dd' % (prefix, num_digits)
...
def render_scene(args,
  ...
  # ---------- Initialise scenes and put in objects
  ...
  cam_trajectory = utils.compute_camera_traj([x for x in camera.location],
                                             args.num_views)

  ###############################################################
  for vid, viewpoint in enumerate(cam_trajectory):
    camera.location = Vector(viewpoint)
    render_args.filepath = os.path.splitext(output_image)[0]+'_{:02d}'.format(vid)+'.png'
    ...
    for _oid, obj in enumerate(objects):
        pixel_coords = utils.get_camera_coords(camera, Vector(obj['3d_coords']))
        objects[_oid]['pixel_coords'].append(pixel_coords)

    # # Render the scene and dump the scene data structure
    scene_struct['objects'] = objects
    if 'location' not in scene_struct['camera'].keys():
      scene_struct['camera']['location'] = []
    scene_struct['camera']['location'].append([v for v in camera.location])
...
def add_random_objects(scene_struct, num_objects, args, camera):
  ...
  for i in range(num_objects):
    ...
    if obj_name in ['Teapot', 'Horse', 'Dolphin']:
      # size_name = 'large'
      r = math.sqrt(2)

    # Choose random orientation for the object.
    theta = 4 * (random.random()-0.5) + random.randint(0, 11) * 30.0 # [0.0, 60.0, 360.0]
...
def render_object_masks(blender_objects, pathfix='masks/'):
  ...
  for i, obj in enumerate(blender_objects):
    ...
    if not os.path.isdir(pathfix):
      os.makedirs(pathfix)
    obj_mk_path = os.path.join(pathfix, '{:03d}.png'.format(i))
...
if __name__ == '__main__':
  if INSIDE_BLENDER:
    ...
    assert args.num_views < 100, 'allows maximum 99 observations per scene'
    main(args)
